...
def pad_dim(dim_list) -> list:
    output = []
    for d in range(len(dim_list) + 4):
        output.append([[0] * (len(dim_list[0]) + 4)] * 2)
        if d <= 1 or d >= len(dim_list) + 2:
            for _ in range(len(dim_list[0])):
                output[d].append([0] * (len(dim_list[0][0]) + 4))
        else:
            for line in dim_list[d - 2]:
                output[d].append([0] * 2 + line + [0] * 2)
        for _ in range(2):
            output[d].append([0] * (len(dim_list[0]) + 4))
